...
def polygon_reduction(vertices, triangles, target_vertices=50000):
        ...
        # Save final mesh
        m = ms.current_mesh()
        ms.save_current_mesh('terrainTest2.ply')
...
class SubTerrain1:
    def __init__(self, terrain_name="terrain", width=256, length=256, vertical_scale=1.0, horizontal_scale=1.0):
        ...
        corner_heights = np.array([[0, 20], [20, 20]])  # Heights at [(0,0), (0,maxY)], [(maxX,0), (maxX,maxY)]
        print(f'max i: {width}')
        for i in range(self.num_rows):
            print(i)

            for j in range(self.num_cols):
                # Compute the normalized position in the heightmap
                x_norm = i / (self.num_rows - 1)
                y_norm = j / (self.num_cols - 1)

                # Linear interpolation for each axis
                top_interp = np.interp(x_norm, [0, 1], corner_heights[1, :])
                bottom_interp = np.interp(x_norm, [0, 1], corner_heights[0, :])

                # Final interpolation between top and bottom
                self.height_field_raw[i, j] = np.interp(y_norm, [0, 1], [bottom_interp, top_interp])
...
def _create_ground_plane():
        ...
        terrain_width = 200 # terrain width [m]
        ...
        width, length = int(terrain_width / horizontal_scale), int(terrain_length / horizontal_scale)

        # Initialize the heightfield
        heightfield = np.zeros((width, length), dtype=np.float32)
        corner_heights = np.array([[0, 20], [20, 20]])  # Heights at [(0,0), (0,maxY)], [(maxX,0), (maxX,maxY)]
        print(f'max i: {width}')
        for i in range(width):
            ...
            for j in range(length):
                ...
                # Final interpolation between top and bottom
                heightfield[i, j] = np.interp(y_norm, [0, 1], [bottom_interp, top_interp])
        def new_sub_terrain(): return SubTerrain1(width=terrain_width,length=terrain_length,horizontal_scale=horizontal_scale,vertical_scale=vertical_scale)
        ...
        terrain = gaussian_terrain(terrain,5,1.0)
        terrain = gaussian_terrain(terrain,0.8,0.2)
        terrain = gaussian_terrain(terrain,200,10)
        terrain = gaussian_terrain(terrain,50,10)
        ...
        vertices, triangles = polygon_reduction(vertices, triangles, target_vertices=600000)
        return vertices, triangles
